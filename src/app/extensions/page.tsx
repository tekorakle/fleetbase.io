...
export default function ExtensionsMarketplacePage() {
 ...
 <Button size="lg" variant="ghost">View Registry Source</Button>
 </Link>
 </div>
 ...
 <div className="text-sm text-muted-foreground">Extensions Available</div>
 </div>
 <div>
 ...
 <div className="text-sm text-muted-foreground">Core Extensions</div>
 </div>
 <div>
 <div className="text-3xl font-bold mb-1">Open</div>
 <div className="text-sm text-muted-foreground">Publisher Platform</div>
 </div>
 </div>
 </div>
 </section>
 ...
 Find extensions that match your operational needs across every area of your logistics business.
 </p>
 </div>
 ...
 <span className="ml-2">For Operators</span>
 </div>
 ...
 <div className="inline-flex items-center rounded-full border px-3 py-1 text-xs mb-4">
 <span className="text-primary">●</span>
 <span className="ml-2">For Developers & Publishers</span>
 </div>
 <h2 className="text-3xl md:text-4xl font-bold mb-6">
 Build Once. Sell to Thousands of Logistics Operators.
 </h2>
 <p className="text-lg text-muted-foreground mb-8">
 The Fleetbase developer platform gives you everything you need to build, publish, and monetize extensions. Reach a global audience of logistics operators without managing your own distribution or payment infrastructure.
 </p>
 <div className="space-y-6">
 {[
 { icon: '👤', title: 'Developer Account & Credentials', desc: 'Create a developer account and generate registry credentials to authenticate your publishing pipeline. Manage multiple extensions from a single account.' },
 { icon: '📝', title: 'Extension Builder & Bundles', desc: 'Create detailed extension listings with descriptions, screenshots, and categorization. Package multiple versions or tiers as bundles for maximum flexibility.' },
 { icon: '💰', title: 'Built-In Monetization', desc: 'Connect your Stripe account and set your own pricing. The marketplace handles payment processing, license management, and access control automatically.' },
 { icon: '📊', title: 'Analytics & Revenue Tracking', desc: 'Track installs, active users, revenue, and conversion rates from your developer dashboard. Understand how your extensions are performing in real time.' },
 ].map((item, i) => (
 <div key={i} className="flex gap-4">
 <div className="flex-shrink-0 w-12 h-12 rounded-lg bg-primary/10 flex items-center justify-center text-2xl">{item.icon}</div>
 <div>
 ...
 <div className="inline-flex items-center rounded-full border px-3 py-1 text-xs mb-4">
 <span className="text-primary">●</span>
 <span className="ml-2">Extension Architecture</span>
 </div>
 <h2 className="text-3xl md:text-4xl font-bold mb-6">A Full-Stack Extension Framework</h2>
 <p className="text-lg text-muted-foreground max-w-3xl mx-auto">
 Fleetbase extensions are full-stack modules with a frontend Ember.js engine and a backend Laravel package — giving you complete control over both the UI and the server-side logic.
 </p>
 </div>
 ...
 <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
 Explore some of the most popular extensions used by logistics operators on the Fleetbase platform.
 </p>
 </div>
 ...
 <span className={`text-xs px-2 py-1 rounded-full border ${ext.tag === 'Free' ? 'text-green-600 border-green-200 bg-green-50 dark:bg-green-950/20' : 'text-primary border-primary/20 bg-primary/5'}`}>
 {ext.tag}
 </span>
 </div>
 ...
 <Link href="https://console.fleetbase.io" target="_blank" rel="noopener noreferrer">
 <Button size="lg">Browse All Extensions</Button>
 </Link>
 </div>
 </div>
 </section>
 ...
 Browse the marketplace to find extensions that fit your workflow, or start building your own and reach thousands of logistics operators.
 </p>
